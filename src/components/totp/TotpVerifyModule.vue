...
const loadPage = async (code: string) => {
  ...
  try {
    const response = await axios.post(`/api/coauth/module/totp/verification/view`, payload);
...
const verifyRequest=async () => {
  ...
  try {
    totpCode.value='';
    const response = await axios.post(`/api/coauth/module/totp/verification/authenticate`, payload);
    if (response.status === 200) {
  ...
  <div class="w-full " v-if="showModule">
    <div class="flex flex-col justify-center items-center mx-4">
      ...
      <div class="mx-12 w-full">
        ...
        <input type="text" v-model="totpCode" name="totpCode" id="totpCode" max="6" :maxlength=6
        ...
        <p id="helper-text-explanation" class="mt-2 text-sm text-gray-700 dark:text-gray-100 mb-6 ">Note: Use your
          authenticator app to get the verification code. The authenticator code in your app resets every 30 seconds</p>
      </div>
